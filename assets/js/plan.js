...
async function checkAvailableZipCode(zipParam) {
    ...
    if (zipCode.length === 0) {
        ...
        $("#textAvailableArea").html(
            `<h2 style='font-size: 24px;'>Please fill your city zipcode</h2>`
        );

        return;
    }

    const data = await fetchDynamicAPI("checkAvailableZipCode", {
        ZipOrCityName: zipCode,
    });
    if (parseInt(data[0].result) === 1) {
        ...
        $("#textAvailableArea").html(
            `<p>Our pricing plan for you</p> <h2 style='font-size: 24px;' >Great! Our service is available in your area.</h2>`
        );
    } else {
        $("#alertAvailable").show();
        $("#choosePlanBlock").hide();
        $("#choosePlanOption").hide();
        $("#textAvailableArea").html(
            "<h2 style='font-size: 24px;'>Your area is not available now</h2>"
        );
    }
}
            ...
            return `
                <div class="col-xl-4 col-lg-4 col-md-6 col-sm-10">
                    <div class="single-card text-center mb-30">
                        <div class="card-top">
                            <p>Single Package</p>
                            <h4>${x.ConnectionType.replace(
                "Connection",
                "<br /> Connection"
            )}</h4>
                        </div>
                        <div class="card-mid">
                            <h4>\$<span style="color: #ff3e3f; font-size: 30px; font-weight: 800; padding-right: 3px;" data-plan-id-${x.Id
                }>${x.Amount.toFixed(2)}</span></h4>
                        </div>
                        <div class="card-bottom">
                            ...
                            <a style="cursor:pointer" onclick="selectPlan(${x.Id
                })" class="borders-btn">Choose this plan</a>
                        </div>
                    </div>
                </div>
        `;
...
async function selectPlan(PlanId) {
    ...
    $("#oneTimeTotal").text(
        parseFloat(connectionTypeCharge) + parseFloat(taxStandard)
    );

    connectType = PlanId;

    const dataSelect = await fetchDynamicAPI("getPlanOptionByPlanId", {
        PlanId: PlanId,
    });
    productForPlan = await fetchDynamicAPI("getProductForPlan", {
        PlanId: PlanId,
    });
    ...
    $("#planOption").html(
        `<option value="0" selected disabled hidden>Choose here</option>` + html
    );
    $("#ProductPlan").html(
        `<option value="0" selected>I have for my own</option>` + htmlOptionProduct
    );
}
async function getPlanDetail(elem) {
    ...
    connectDetail = 0;
    const data = await fetchDynamicAPI("getPlanDetailByPlanOptionId", {
        PlanOptionId: parseInt(elem.value),
    });
...
function onSelectProduct(elem) {
    ...
    if (productId === 0) {
        ...
        // console.log();
        $("#oneTimeTotal").text(
            parseFloat($("#standardInstall").text()) +
            parseFloat($("#standardInstallTax").text())
        );
        $("#productChoosePlan").html("");
        return;
    }
    ...
    for (let i = 0; i < productForPlan.length; i++) {
        if (parseInt(elem.value) === productForPlan[i].Id) {
            ...
            $("#oneTimeTotal").text(
                parseFloat(standardInstall) + productForPlan[i].Price + taxStandInstall
            );
...
async function placeOrder() {
    $("#firstName").removeClass("single-input-alert");
    $("#lastName").removeClass("single-input-alert");
    $("#email").removeClass("single-input-alert");
    $("#phone").removeClass("single-input-alert");
    $("#address").removeClass("single-input-alert");
    $("#customerCity").css("border", "");
    $("#customerPaymentMethod").css("border", "");
    ...
    const paymentMethod = $("#customerPaymentMethod").val() || "";
    ...
    if (
        ...
        if (paymentMethod.length === 0) {
            $("#customerPaymentMethod").css("border", "1px solid red");
        }
        return;
    }
    ...
    const formData = {
        ...
        CityId: customerCity,
        PlanId: connectType,
        PlanOptionId: connectOption,
        PlanDetailId: connectDetail,
        ProductId: productId,
    ...
    if (data[0].CustomerId.length === 10) {
        $("#choosePlanOption").hide();
        $("#textAvailableArea").html(
            `<h2 style="font-size: 24px;">Order successful, your customer code is: ${data[0].CustomerId}.</h2>`
        );
    }
}
